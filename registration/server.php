...
if (isset($_POST['reg_user'])) {
	...
	if (empty($city)) {
		array_push($errors, "Ville requis");
	}
	if (empty($cp)) {
		array_push($errors, "Code postal requis");
	}
	if (empty($password_1)) {
	...
	if (count($errors) == 0) {
		...
		$query = "INSERT INTO users (username, email, cp, city, password) 
					  VALUES('$username', '$email', '$cp', '$city', '$password')";
